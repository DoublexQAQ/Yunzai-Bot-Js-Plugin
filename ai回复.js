...
export class AI extends plugin {
  ...
  async AI(e) {
    if (!e.msg || typeof e.msg !== 'string' || e.msg.startsWith('[CQ:image')) {
      return false;
    }
    if (e.msg.startsWith('#')) {
      return false;
    ...
    try {
      ...
      const responseJson = await response.json(); // 解析为 JSON 格式
      if (responseJson.code === 1) {
        const finalResponse = responseJson.data[1].content.trim(); // 获取第二个content
        await e.reply(finalResponse,true);
      } else {
        throw new Error("API返回错误：" + responseJson.message);
      }
    } catch (error) {
